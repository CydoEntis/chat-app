...
const Message = (props) => {
	...
	if (props.sending && !props.lastMessage) {
		isNewTurn =
			!props.chat.last_message.sender || props.chat.last_message.sender.username !== senderUsername;
	}

	const messageDateTime = getDateTime(props.message.created);
	...
	return (
		<div
			...
			{isNewTurn && (
				<div style={{ height: '0px' }}>
					<div
						...
						<div style={styles.avatarText}>{senderUsername.substr(0, 2).toUpperCase()}</div>
					</div>
				</div>
			)}
			...
			<div
				...
				<MessageBody text={props.message.text} />
			</div>
		</div>
	);
};
